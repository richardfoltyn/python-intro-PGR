...
\item
  ...
  the terminal period \(T\) and iterating forward through periods
...
with aggregate uncertainty such as Krusell-Smith (1998)).
    ...
    Note that for numerical purposes it is necessary to formulate the
household problem in recursive form. You may be more familiar with the
sequential formulation of an infinite horizon problem which could look
something like the following, \[
\begin{aligned}
V(a_0) &= \max_{(c_t)_{t=0}^{\infty}, (a_{t+1})_{t=0}^{\infty}}
\sum_{t=0}^{\infty} \beta^t u(c_t)  \\
\text{s.t.} \quad & c_t + a_{t+1} = (1+r)a_t + y_t \quad \forall~t \\
& c_t \geq 0,\; a_{t+1} \geq 0 \quad \forall~t
\end{aligned}
\] where the household chooses sequences of consumption
...
given \(a\) and parameters.
    ...
    \hypertarget{solving-household-problems-with-vfi}{%
\section{Solving household problems with
VFI}\label{solving-household-problems-with-vfi}}

    \hypertarget{no-income-uncertainty}{%
\subsection{No income uncertainty}\label{no-income-uncertainty}}
...
\hypertarget{grid-search-algorithm}{%
\subsubsection{Grid search algorithm}\label{grid-search-algorithm}}
...
\(N\) feasible asset levels \[
\Gamma_{a} = \Bigl\{a_1, a_1, a_2, \dots, a_N \Bigr\}
...
value function \(V(a')\).
    ...
    The grid search algorithm for this problem can thus be summarized as
follows:

\begin{enumerate}
\def\labelenumi{\arabic{enumi}.}
\item
  Create asset grid \(\Gamma_a = (a_1, \dots, a_{N_a})\)
\item
  Pick initial guess for value function, \(V_0\)
\item
  In iteration \(n\), perform the following steps:

  For each asset level \(a_i\),
  ...
  \item
    Set \(V_{n+1}(a_i) = U_k\) and store \(k\) as the optimal choice at
    \(a_i\)
  \end{enumerate}
\end{enumerate}
